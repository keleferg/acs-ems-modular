...
begin
  ...
  select
    nullif(trim(x.obj->>'type_certificate_holder'), '') as type_certificate_holder,
    nullif(trim(x.obj->>'civil_model_designation'), '') as civil_model_designation,
    nullif(trim(x.obj->>'prior_model_designation'), '') as prior_model_designation,
    nullif(trim(x.obj->>'equivalent_military_designation'), '') as equivalent_military_designation,
    trim(x.obj->>'type_rating_designation') as type_rating_designation,
    coalesce(nullif(x.obj->>'sort_order','')::integer, x.ord::integer * 10) as sort_order
  ...
  delete from public.faa_type_rating_aircraft where id is not null;
