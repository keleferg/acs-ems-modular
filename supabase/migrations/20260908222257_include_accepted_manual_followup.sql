create or replace function public.applicant_respond_to_practical_test_followup(
  p_request_id uuid,
  p_action text
)
...
begin
  ...
  if p_action = 'remain_active' then
    ...
    set
      email_status =
        case
          when sent_at is not null then 'responded'
          else email_status
        end,
      response_action = 'remain_active',
      response_source = 'portal',
      responded_at = v_now
    where practical_test_request_id = p_request_id
...
create or replace function public.service_respond_to_practical_test_followup(
  p_token_hash text,
  p_action text
)
returns table(
...
as $function$
declare
  v_followup public.practical_test_request_followups%rowtype;
...
begin
  ...
  where id = v_followup.practical_test_request_id
  for update;
