create or replace function public.import_faa_type_rating_aircraft_once(p_secret text, p_rows jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select
    nullif(trim(elem ->> 'type_certificate_holder'), '') as type_certificate_holder,
    nullif(trim(elem ->> 'civil_model_designation'), '') as civil_model_designation,
    nullif(trim(elem ->> 'prior_model_designation'), '') as prior_model_designation,
    nullif(trim(elem ->> 'equivalent_military_designation'), '') as equivalent_military_designation,
    trim(elem ->> 'type_rating_designation') as type_rating_designation,
    coalesce(nullif(elem ->> 'sort_order', '')::integer, ord::integer * 10) as sort_order
  from jsonb_array_elements(p_rows) with ordinality as j(elem, ord);
  ...
  with src as (
    select r.*,
      (
        select string_agg(
          regexp_replace(trim(piece), '\s+', '', 'g'),
          ';'
          order by regexp_replace(trim(piece), '\s+', '', 'g')
        )
