...
export function ExaminerSidebar({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function loadAccount() {
      ...
      if (profileResult.error) {
        console.error("Unable to load portal profile:", profileResult.error);
      }

      if (rolesResult.error) {
        console.error("Unable to load portal roles:", rolesResult.error);
      }

      const loadedRoles = (rolesResult.data ?? []).map((row) => row.role);

      if (
        !loadedRoles.includes("examiner") &&
        !loadedRoles.includes("administrator")
      ) {
    ...
    <div className="flex h-full flex-col bg-slate-950 text-white">
      <div className="border-b border-slate-800 px-5 py-5">
        <Link
          href="/examiner/dashboard"
          className="block"
          aria-label="Aviation Training Solutions Examiner Portal"
        >
          <Image
        ...
        <div className="relative">
          ...
          {portalMenuOpen ? (
            <div className="absolute left-0 right-0 top-full z-40 mt-2 overflow-hidden rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
              <Link
                href="/examiner/dashboard"
                className="flex items-center gap-3 bg-amber-500/10 px-4 py-3 text-sm font-semibold text-amber-300"
              >
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-4">
        {examinerNavigation.map((item) => {
          const Icon = item.icon;
          const active =
            isNavigationItemActive(pathname, item.href) ||
            (item.href === "/examiner/settings" &&
              pathname === "/examiner/availability");
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur lg:hidden">
          <div className="flex h-16 items-center justify-between px-4">
            ...
            <p className="text-sm font-bold text-slate-900">Examiner Portal</p>
